...
function keyCheck(key: eventCodeType | string): eventCodeType {
    ...
    return key as eventCodeType
}
function keyStringParse(keyString: string): eventCodeType[] {
    const noWhiteSpace = keyString.replace(" ", "")
    const seperate = noWhiteSpace.split("+")
    seperate.forEach(x => keyCheck(x))
    return seperate as eventCodeType[]
}
...
function flicker(keybind: eventCodeType | string,onkeydown: () => {}, onkeyup: () => {}, keybindId?: string): () => void {
    let triggered: boolean = false
    const isFuncKeydown: boolean = typeof onkeydown === "function"
    const isFuncKeyup: boolean = typeof onkeyup === "function"

    const keyComparison = (): string => {
